...
if [[ "$(git branch --show-current)" != "master" ]]; then
	echo "Error: dynamicaviation must be on the master branch." >&2
	exit 1
fi
...
$plugin_path = $repo_dir . '/dynamicaviation.php';
...
if (!is_string($plugin)) {
	throw new RuntimeException('Unable to read dynamicaviation.php.');
}
...
$plugin = $replace_once(
	$plugin,
	"/(define\([ \\t]*'DYNAMICAVIATION_VERSION'[ \\t]*,[ \\t]*')\\d+\\.\\d+\\.\\d+('[ \\t]*\);)/",
	static fn(array $matches): string => $matches[1] . $new_version . $matches[2],
	'DYNAMICAVIATION_VERSION'
);
