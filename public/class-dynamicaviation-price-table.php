			...
			while ($wp_query->have_posts() )
			{
				...
				for($x = 0; $x < count($table_price); $x++)
				{
					...
					if($show_all && $origin_iata !== $destination_iata && !empty($origin_iata) && !empty($destination_iata))
					{
						...
						$row .= '<td><strong>'.esc_html('$'.$this->utilities->money_format($price)).'</strong><br/><span class="text-muted">';
						...
						if(floatval($fees) > 0)
						{
							$row .= '<br/><span class="text-muted">' . esc_html(__('Fees per pers.', 'dynamicaviation').' '.'$'.$this->utilities->money_format($fees)) . '</span>';
						}
