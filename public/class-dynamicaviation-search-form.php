...
class Dynamic_Aviation_Search_Form {
    ...
    public function search_form($is_two_cols = true)
    {
		...
		ob_start(); 
        ?>
            <form class="aircraft_search_form" data-hash-params="<?php echo esc_attr(implode(',', $this->utilities->search_form_hash_param_names()));?>" data-method="post" data-action="<?php echo esc_attr(base64_encode(home_lang().'instant_quote')); ?>" autocomplete="off" data-gclid="true">
