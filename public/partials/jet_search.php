...
if ($wp_jet_search->have_posts())
{
	...
	$request .= '</span>';	
	$request .= '</h3>';
	$request .= '<h3>'.esc_html(__('Passengers', 'dynamicaviation')).': <span class="linkcolor">'.esc_html(sanitize_text_field($_GET['jet_pax'])).'</span></h3>';
	$table = null;
		...
		for($x = 0; $x < count($table_price); $x++)
		{
			if(($_GET['jet_origin'] == $table_price[$x][0] && $_GET['jet_destination'] == $table_price[$x][1]) || ($_GET['jet_origin'] == $table_price[$x][1] && $_GET['jet_destination'] == $table_price[$x][0]))
			{
				...
				if(Charterflights_Meta_Box::jet_get_meta( 'jet_commercial' ) != 0)
				{
					...
					$price_row = '<td><small class="text-muted">USD</small><br/><strong class="large">'.esc_html('$'.number_format($price, 0, '.', ',')).'</strong><br /><span class="small text-muted">'.esc_html('$'.number_format(($price / floatval(sanitize_text_field($_GET['jet_pax']))), 0, '.', ',')).' '.esc_html(__('Per Person', 'dynamicaviation')).'</span>';
				...
				else
				{
					...
					$flight_desc .= '<br/>';
					$flight_desc .= '<small>'.esc_html(Dynamic_Aviation_Public::jet_type(Charterflights_Meta_Box::jet_get_meta( 'jet_type' ))).'</small>';
	...
	<table class="margin-bottom pure-table pure-table-bordered text-center instant_quote_table small">
		<thead>
			<tr>
				<th><?php echo esc_html(__('Flights', 'dynamicaviation')); ?></th>
			...
			<div class="modal-header clearfix">
				...
				<span class="close pointer pull-right large"><i class="fas fa-times"></i></span>
					...
					<div class="pure-g gutters">
						<div class="pure-u-1 pure-u-md-1-2">
							<div class="bottom-20">
								<label for="lead_name"><?php echo esc_html(__('Name', 'dynamicaviation')); ?></label>
								<input type="text" name="lead_name" />								
							</div>
						</div>
						<div class="pure-u-1 pure-u-md-1-2">
							<div class="bottom-20">
								<label for="lead_lastname"><?php echo esc_html(__('Last Name', 'dynamicaviation')); ?></label>
					...
					<div class="pure-g gutters">
						<div class="pure-u-1 pure-u-md-1-2">
							<div class="bottom-20">
								<label for="lead_email"><?php echo esc_html(__('Email', 'dynamicaviation')); ?></label>
								<input type="email" name="lead_email" />								
							</div>
						</div>
						<div class="pure-u-1 pure-u-md-1-2">
							<div class="bottom-20">
								<label for="lead_phone"><?php echo esc_html(__('Phone', 'dynamicaviation')); ?></label>
					...
					<div class="pure-g gutters">
						<div class="pure-u-1 pure-u-md-1-2">
							<div class="bottom-20">
								<label for="country"><?php echo esc_html(__('Country', 'dynamicaviation')); ?></label>
								<select name="lead_country" class="countrylist"><option>--</option></select>								
							</div>
						</div>
						<div class="pure-u-1 pure-u-md-1-2">
							<div class="bottom-20">
								<label for="lead_children"><?php echo esc_html(__('Traveling With Children', 'dynamicaviation')); ?>?</label>
								<select name="lead_children">
									<option value="no"><?php echo esc_html(__('No', 'dynamicaviation')); ?></option>
									<option value="yes"><?php echo esc_html(__('Yes', 'dynamicaviation')); ?></option>
								</select>								
							</div>
						</div>
					</div>				
				...
				<?php if(get_option('captcha_site_key')): ?>
					<?php if(get_option('captcha_site_key') != null): ?>
						<button data-badge="bottomleft" data-callback="validate_request_quote" class="g-recaptcha pure-button pure-button-primary" id="jet_request" data-sitekey="<?php echo esc_html(get_option('captcha_site_key')); ?>"><i class="fas fa-plane"></i> <?php echo esc_html(__('Send Request', 'dynamicaviation'));?></button>	
					<?php endif; ?>
				<?php endif; ?>				
